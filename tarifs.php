<!DOCTYPE html>
<html lang="ru">
<head>
  <meta name="viewport" content="width=device-width, initial-scale=1" />
  <meta charset="UTF-8">
  <title>Бухгалтерский успех - Тарифы</title>
  <link rel="stylesheet" href="css/all.css" type="text/css" />
  <link rel="stylesheet" href="css/animate.css" type="text/css"/>
  <!--[if lt IE 9]>
    <script src="http://html5shim.googlecode.com/svn/trunk/html5.js"></script>
  <![endif]-->
  <!--[if lte IE 8]>
    <link rel="stylesheet" href="css/all-old-ie.css">
  <![endif]-->
  <!--[if gt IE 8]>
    <link rel="stylesheet" href="css/all.css">
  <![endif]-->
  <script src="//maps.googleapis.com/maps/api/js?v=3.exp"></script>
  <script type="text/javascript" src="js/infobox.js"></script>
  <script src="//code.jquery.com/jquery-1.11.3.min.js"></script>
  <!--<script src="js/jquery.parallax.min.js"></script>-->
  <!--<script type="text/javascript" src="js/bg-loaded.js"></script>
  <script type="text/javascript" src="js/jquery.bxslider.js"></script>
  <script type="text/javascript" src="js/jquery.scrollTo.min.js"></script>-->
  <script type="text/javascript" src="js/jquery.placeholder.js"></script>
  <script type="text/javascript" src="js/jquery.colorbox-min.js"></script>
  <script type="text/javascript" src="js/script-easy.js"></script>
  <script type="text/javascript" src="js/gmap.js"></script>
  
</head>
<body>
  <header class="header">
    <nav class="topmenu">
      <div class="topmenu_inner container">
        <ul class="topmenu_list">
          <li class="topmenu_item">
            <a href="/main.php" class="topmenu_link">Главная</a>
          </li>
          <li class="topmenu_item">
            <a href="/about.php" class="topmenu_link">О компании</a>
          </li>
          <li class="topmenu_item">
            <span class="topmenu_link">Наши услуги</span>
            <div class="topmenu_wrap">
              <ul class="topmenu_list__l2">
                <li class="topmenu_item__l2">
                  <a href="/service.php" class="topmenu_link__l2">Восстановление учёта</a>
                </li>
                <li class="topmenu_item__l2">
                  <a href="/service.php" class="topmenu_link__l2">Постановка учёта</a>
                </li>
                <li class="topmenu_item__l2">
                  <a href="/service.php" class="topmenu_link__l2">Полное ведение учёта</a>
                </li>
                <li class="topmenu_item__l2">
                  <a href="/service.php" class="topmenu_link__l2">Составление отчётности</a>
                </li>
                <li class="topmenu_item__l2">
                  <a href="/service.php" class="topmenu_link__l2">Консультирование</a>
                </li>
                <li class="topmenu_item__l2">
                  <a href="/service.php" class="topmenu_link__l2">Подготовка документов</a>
                </li>
                <li class="topmenu_item__l2">
                  <a href="/service.php" class="topmenu_link__l2">Доступ к программе 1С</a>
                </li>
                <li class="topmenu_item__l2">
                  <a href="/service.php" class="topmenu_link__l2">Документы для кредитов</a>
                </li>
                <li class="topmenu_item__l2">
                  <a href="/service.php" class="topmenu_link__l2">Налоговая оптимизация</a>
                </li>
                <li class="topmenu_item__l2">
                  <a href="/service.php" class="topmenu_link__l2">Учёт оплаты труда</a>
                </li>
                <li class="topmenu_item__l2">
                  <a href="/service.php" class="topmenu_link__l2">Персонифицированный учёт</a>
                </li>
                <li class="topmenu_item__l2">
                  <a href="/service.php" class="topmenu_link__l2">Годовая отчётность</a>
                </li>
              </ul>
            </div>
          </li>
          <li class="topmenu_item">
            <a href="/price.php" class="topmenu_link">Цены</a>
          </li>
          <li class="topmenu_item">
            <a href="/service.php" class="topmenu_link">Гарантии</a>
          </li>
          <li class="topmenu_item">
            <a href="/tarifs.php" class="topmenu_link topmenu_link__active">Тарифы</a>
          </li>
          <li class="topmenu_item">
            <a href="/infos.php" class="topmenu_link">Полезная информация</a>
          </li>
          <li class="topmenu_item">
            <a href="/contact.php" class="topmenu_link">Контакты</a>
          </li>
        </ul>
      </div>
    </nav>
    <div class="header_bottom container">
      <div class="header_section">
        <div class="logo">
          <a class="logo_link" href="/main.php">Бухгалтерский успех</a>
          <span class="logo_text">Бухгалтерские услуги №1 в Беларуси</span>
        </div>
      </div>
      <div class="header_section">
        <div class="address">
          <div class="address_left">
            <p>+375 <span>(17)</span> 385 94 80</p>
            <p>+375 (29) 385 94 80</p>
            <p>+375 (29) 335 94 80</p>
          </div>
          <div class="address_right">
            <p>ул. Притыцкого, 83</p>
            <p>пом. 33н, каб. 2</p>
            <p>Минск, 220140, Беларусь</p>
          </div>
        </div>
      </div>
      <div class="header_section">
        <a id="b_callback" class="button button__phone" href="/">Заказать звонок</a>
      </div>
    </div>
  </header>

  <div class="title title__tarifs">
    <div class="title_wrap container">
      <h1 class="title_h1">Тарифы</h1>
      <div class="title_descr">
        <p>При формировании каждого тарифного плана учитывается перечень и количество обрабатываемых первичных документов, штатный состав сотрудников, наличие внешнеэкономической деятельности и другие данные клиентам.</p>
      </div>
    </div>  
  </div>

  <div class="mybreadcrumb container">
    <a class="mybreadcrumb_link" href="/main.php">Главная</a>
    <span class="mybreadcrumb_item">Тарифы</span>
  </div>

  <div class="tarifs container">
    <div class="tarifs_wrap">
      <a href="/main.php#main2" class="tarifs_link">Перейти к калькулятору расчёта цен</a>
      <div class="tarifs_title">Тарифы на пакеты бухгалтерских услуг</div>
    </div>
    <div class="tarifs_wrap">
      <div class="tarifs_item">
        <div class="tarif-block">
          <div class="tarif-block_title">Начальный</div>
          <div class="tarif-block_price">1 000 000 бел.руб.</div>
          <ul class="tarif-block_list">
            <li class="tarif-block_item">
              <div class="tarif-block_wrap">
                <div class="tarif-block_left">Расчет и начисление заработной платы, расчет налогов, отчислений, подготовка и  предоставление отчетности</div>
                <div class="tarif-block_right">До 2 сотрудников</div>
              </div>
            </li>
            <li class="tarif-block_item">
              <div class="tarif-block_wrap">
                <div class="tarif-block_left">Ведение учета на основании первичных документов (внесение в базу 1С)</div>
                <div class="tarif-block_right">До 50 в месяц</div>
              </div>
            </li>
            <li class="tarif-block_item">
              <div class="tarif-block_wrap">
                <div class="tarif-block_left">Ведение бухгалтерских и налоговых регистров</div>
              </div>
            </li>
            <li class="tarif-block_item">
              <div class="tarif-block_wrap">
                <div class="tarif-block_left">Ежеквартальные налоговые декларации</div>
              </div>
            </li>
            <li class="tarif-block_item">
              <div class="tarif-block_wrap">
                <div class="tarif-block_left">Ежеквартальный отчёт в Фонд социальной защиты населения</div>
              </div>
            </li>
            <li class="tarif-block_item">
              <div class="tarif-block_wrap">
                <div class="tarif-block_left">ПУ-1, ПУ-2, ПУ-3</div>
              </div>
            </li>
            <li class="tarif-block_item">
              <div class="tarif-block_wrap">
                <div class="tarif-block_left">Годовой отчет Белгосстрах</div>
              </div>
            </li>
          </ul>
          <a href="/tarif.php" class="tarif-block_link">Подробнее о тарифе</a>
          <a href="" class="button button__tarifsorder">Заказать тариф</a>
        </div>
      </div>
      <div class="tarifs_item">
        <div class="tarif-block">
          <div class="tarif-block_title">Активный</div>
          <div class="tarif-block_price">2 000 000 бел.руб.</div>
          <ul class="tarif-block_list">
            <li class="tarif-block_item">
              <div class="tarif-block_wrap">
                <div class="tarif-block_left">Расчет и начисление заработной платы, расчет налогов, отчислений, подготовка и  предоставление отчетности</div>
                <div class="tarif-block_right">До 5 сотрудников</div>
              </div>
            </li>
            <li class="tarif-block_item">
              <div class="tarif-block_wrap">
                <div class="tarif-block_left">Ведение учета на основании первичных документов (внесение в базу 1С)</div>
                <div class="tarif-block_right">До 100 в месяц</div>
              </div>
            </li>
            <li class="tarif-block_item">
              <div class="tarif-block_wrap">
                <div class="tarif-block_left">Ведение бухгалтерских и налоговых регистров</div>
              </div>
            </li>
            <li class="tarif-block_item">
              <div class="tarif-block_wrap">
                <div class="tarif-block_left">Ежеквартальные налоговые декларации</div>
              </div>
            </li>
            <li class="tarif-block_item">
              <div class="tarif-block_wrap">
                <div class="tarif-block_left">Ежеквартальный отчёт в Фонд социальной защиты населения</div>
              </div>
            </li>
            <li class="tarif-block_item">
              <div class="tarif-block_wrap">
                <div class="tarif-block_left">ПУ-1, ПУ-2, ПУ-3</div>
              </div>
            </li>
            <li class="tarif-block_item">
              <div class="tarif-block_wrap">
                <div class="tarif-block_left">Годовой отчет Белгосстрах</div>
              </div>
            </li>
          </ul>
          <a href="/tarif.php" class="tarif-block_link">Подробнее о тарифе</a>
          <a href="" class="button button__tarifsorder">Заказать тариф</a>
        </div>
      </div>
      <div class="tarifs_item">
        <div class="tarif-block">
          <div class="tarif-block_title">Рациональный</div>
          <div class="tarif-block_price">3 000 000 бел.руб.</div>
          <ul class="tarif-block_list">
            <li class="tarif-block_item">
              <div class="tarif-block_wrap">
                <div class="tarif-block_left">Расчет и начисление заработной платы, расчет налогов, отчислений, подготовка и  предоставление отчетности</div>
                <div class="tarif-block_right">До 7 сотрудников</div>
              </div>
            </li>
            <li class="tarif-block_item">
              <div class="tarif-block_wrap">
                <div class="tarif-block_left">Ведение учета на основании первичных документов (внесение в базу 1С)</div>
                <div class="tarif-block_right">До 150 в месяц</div>
              </div>
            </li>
            <li class="tarif-block_item">
              <div class="tarif-block_wrap">
                <div class="tarif-block_left">Ведение бухгалтерских и налоговых регистров</div>
              </div>
            </li>
            <li class="tarif-block_item">
              <div class="tarif-block_wrap">
                <div class="tarif-block_left">Ежеквартальные налоговые декларации</div>
              </div>
            </li>
            <li class="tarif-block_item">
              <div class="tarif-block_wrap">
                <div class="tarif-block_left">Ежеквартальный отчёт в Фонд социальной защиты населения</div>
              </div>
            </li>
            <li class="tarif-block_item">
              <div class="tarif-block_wrap">
                <div class="tarif-block_left">ПУ-1, ПУ-2, ПУ-3</div>
              </div>
            </li>
            <li class="tarif-block_item">
              <div class="tarif-block_wrap">
                <div class="tarif-block_left">Годовой отчет Белгосстрах</div>
              </div>
            </li>
          </ul>
          <a href="/tarif.php" class="tarif-block_link">Подробнее о тарифе</a>
          <a href="" class="button button__tarifsorder">Заказать тариф</a>
        </div>
      </div>
      <div class="tarifs_item">
        <div class="tarif-block">
          <div class="tarif-block_title">Оптимальный</div>
          <div class="tarif-block_price">4 000 000 бел.руб.</div>
          <ul class="tarif-block_list">
            <li class="tarif-block_item">
              <div class="tarif-block_wrap">
                <div class="tarif-block_left">Расчет и начисление заработной платы, расчет налогов, отчислений, подготовка и  предоставление отчетности</div>
                <div class="tarif-block_right">До 10 сотрудников</div>
              </div>
            </li>
            <li class="tarif-block_item">
              <div class="tarif-block_wrap">
                <div class="tarif-block_left">Ведение учета на основании первичных документов (внесение в базу 1С)</div>
                <div class="tarif-block_right">До 200 в месяц</div>
              </div>
            </li>
            <li class="tarif-block_item">
              <div class="tarif-block_wrap">
                <div class="tarif-block_left">Ведение бухгалтерских и налоговых регистров</div>
              </div>
            </li>
            <li class="tarif-block_item">
              <div class="tarif-block_wrap">
                <div class="tarif-block_left">Ежеквартальные налоговые декларации</div>
              </div>
            </li>
            <li class="tarif-block_item">
              <div class="tarif-block_wrap">
                <div class="tarif-block_left">Ежеквартальный отчёт в Фонд социальной защиты населения</div>
              </div>
            </li>
            <li class="tarif-block_item">
              <div class="tarif-block_wrap">
                <div class="tarif-block_left">ПУ-1, ПУ-2, ПУ-3</div>
              </div>
            </li>
            <li class="tarif-block_item">
              <div class="tarif-block_wrap">
                <div class="tarif-block_left">Годовой отчет Белгосстрах</div>
              </div>
            </li>
          </ul>
          <a href="/tarif.php" class="tarif-block_link">Подробнее о тарифе</a>
          <a href="" class="button button__tarifsorder">Заказать тариф</a>
        </div>
      </div>
      <div class="tarifs_item">
        <div class="tarif-block">
          <div class="tarif-block_title">Деловой</div>
          <div class="tarif-block_price">5 000 000 бел.руб.</div>
          <ul class="tarif-block_list">
            <li class="tarif-block_item">
              <div class="tarif-block_wrap">
                <div class="tarif-block_left">Расчет и начисление заработной платы, расчет налогов, отчислений, подготовка и  предоставление отчетности</div>
                <div class="tarif-block_right">До 15 сотрудников</div>
              </div>
            </li>
            <li class="tarif-block_item">
              <div class="tarif-block_wrap">
                <div class="tarif-block_left">Ведение учета на основании первичных документов (внесение в базу 1С)</div>
                <div class="tarif-block_right">До 250 в месяц</div>
              </div>
            </li>
            <li class="tarif-block_item">
              <div class="tarif-block_wrap">
                <div class="tarif-block_left">Ведение бухгалтерских и налоговых регистров</div>
              </div>
            </li>
            <li class="tarif-block_item">
              <div class="tarif-block_wrap">
                <div class="tarif-block_left">Ежеквартальные налоговые декларации</div>
              </div>
            </li>
            <li class="tarif-block_item">
              <div class="tarif-block_wrap">
                <div class="tarif-block_left">Ежеквартальный отчёт в Фонд социальной защиты населения</div>
              </div>
            </li>
            <li class="tarif-block_item">
              <div class="tarif-block_wrap">
                <div class="tarif-block_left">ПУ-1, ПУ-2, ПУ-3</div>
              </div>
            </li>
            <li class="tarif-block_item">
              <div class="tarif-block_wrap">
                <div class="tarif-block_left">Годовой отчет Белгосстрах</div>
              </div>
            </li>
          </ul>
          <a href="/tarif.php" class="tarif-block_link">Подробнее о тарифе</a>
          <a href="" class="button button__tarifsorder">Заказать тариф</a>
        </div>
      </div>
      <div class="tarifs_item">
        <div class="tarif-block">
          <div class="tarif-block_title">Успешный</div>
          <div class="tarif-block_price">6 000 000 бел.руб.</div>
          <ul class="tarif-block_list">
            <li class="tarif-block_item">
              <div class="tarif-block_wrap">
                <div class="tarif-block_left">Расчет и начисление заработной платы, расчет налогов, отчислений, подготовка и  предоставление отчетности</div>
                <div class="tarif-block_right">До 20 сотрудников</div>
              </div>
            </li>
            <li class="tarif-block_item">
              <div class="tarif-block_wrap">
                <div class="tarif-block_left">Ведение учета на основании первичных документов (внесение в базу 1С)</div>
                <div class="tarif-block_right">До 300 в месяц</div>
              </div>
            </li>
            <li class="tarif-block_item">
              <div class="tarif-block_wrap">
                <div class="tarif-block_left">Ведение бухгалтерских и налоговых регистров</div>
              </div>
            </li>
            <li class="tarif-block_item">
              <div class="tarif-block_wrap">
                <div class="tarif-block_left">Ежеквартальные налоговые декларации</div>
              </div>
            </li>
            <li class="tarif-block_item">
              <div class="tarif-block_wrap">
                <div class="tarif-block_left">Ежеквартальный отчёт в Фонд социальной защиты населения</div>
              </div>
            </li>
            <li class="tarif-block_item">
              <div class="tarif-block_wrap">
                <div class="tarif-block_left">ПУ-1, ПУ-2, ПУ-3</div>
              </div>
            </li>
            <li class="tarif-block_item">
              <div class="tarif-block_wrap">
                <div class="tarif-block_left">Годовой отчет Белгосстрах</div>
              </div>
            </li>
          </ul>
          <a href="/tarif.php" class="tarif-block_link">Подробнее о тарифе</a>
          <a href="" class="button button__tarifsorder">Заказать тариф</a>
        </div>
      </div>
    </div>
  </div>

  

  <div class="block-form">
    <div class="block-form_layer"></div>
    <div class="block-form_wrap container">
      <form action="#">
        <div class="block-form_top">
          <div class="block-form_descr">Введите ваше имя и телефон. Специалисты Бухгалтерский успех свяжутся с вами в течение 30 минут</div>
          <div class="block-form_title">Напишите нам письмо или задайте вопрос</div>
        </div>
        <div class="block-form_bottom">
          <div class="block-form_left">
            <div class="block-form_inner">
              <textarea id="edit-submitted-message" class="form-textarea required" rows="5" cols="60" placeholder="Ваш вопрос..." required="required"></textarea>
            </div>
          </div>
          <div class="block-form_right">
            <div class="block-form_inner">
              <input class="form-text" type="text" maxlength="128" size="60" value="" placeholder="Ваше имя">
            </div>
            <div class="block-form_inner">
              <input class="form-text" type="text" maxlength="128" size="60" value="" placeholder="Ваш телефон" required="required">
            </div>
            <div class="block-form_button">
              <input class="webform-submit button-primary form-submit ajax-processed" type="submit" value="Отправить" name="op">
            </div>
          </div>
        </div>
      </form>
    </div>
  </div>

  <footer class="footer">
    <div class="gmap" id="gmap"></div>
    <div class="footer_bottom container">
      <div class="footer_wrap">
        <div class="footer_section">
          <div class="logo logo__footer">
            <a class="logo_link logo_link__footer" href="/">Бухгалтерский успех</a>
          </div>
        </div>
        <div class="footer_section">
          <div class="address address__footer">
            <div class="address_left address_left__footer">
              <p>ул. Притыцкого, 83</p>
              <p>пом. 33н, каб. 2</p>
              <p>Минск, 220140, Беларусь</p>
            </div>
            <div class="address_right address_right__footer">
              <p>+375 <span>(17)</span> 385 94 80</p>
              <p>+375 (29) 385 94 80</p>
              <p>+375 (29) 335 94 80</p>
            </div>
          </div>
        </div>
        <div class="footer_section">
          <div class="social">
            <a href="" class="social_link social_link__vk">ВКонтакте</a>
            <a href="" class="social_link social_link__fb">Фэйсбук</a>
            <a href="" class="social_link social_link__tt">Твиттер</a>
            <a href="" class="social_link social_link__ok">Одноклассники</a>
          </div>
        </div>
      </div>
      <div class="footer_wrap">
        <a href="https://webformat.by/" class="dev">Веб Формат - создание и продвижение сайтов</a>
        <div class="footer_copyright">© ООО "Бухгалтерский успех". Все права защищены.</div>
      </div>
    </div>
  </footer>
  <div class="hide">

    <div id="form1" class="form-popup">
      <div class="form-popup_wrap form-popup_wrap__question">
        <form action="#">
          <div class="form-popup_title">Задать вопрос</div>
          <div class="form-popup_inner">
            <input class="form-text" type="text" maxlength="128" size="60" value="" placeholder="Введите имя">
          </div>
          <div class="form-popup_inner">
            <input class="form-text" type="text" maxlength="128" size="60" value="" placeholder="Введите номер телефона" required="required">
          </div>
          <div class="form-popup_inner">
            <textarea id="edit-submitted-message" class="form-textarea required" rows="5" cols="60" placeholder="Ваш вопрос..." required="required"></textarea>
          </div>
          <div class="form-popup_button">
            <input class="webform-submit button-primary form-submit ajax-processed" type="submit" value="Отправить заявку" name="op">
          </div>
          <div class="form-popup_descr">Введите ваше имя и телефон. Специалисты Бухгалтерский успех свяжутся с вами в течение 30 минут.</div>
        </form>
      </div>
    </div>
    <div id="form2" class="form-popup">
      <div class="form-popup_wrap form-popup_wrap__buh">
        <form action="#">
          <div class="form-popup_title">Подать заявку на бухгалтерское обслуживание</div>
          <div class="form-popup_inner">
            <input class="form-text" type="text" maxlength="128" size="60" value="" placeholder="Введите имя">
          </div>
          <div class="form-popup_inner">
            <input class="form-text" type="text" maxlength="128" size="60" value="" placeholder="Введите телефон" required="required">
          </div>
          <div class="form-popup_inner">
            <textarea id="edit-submitted-message" class="form-textarea required" rows="5" cols="60" placeholder="Введите комментарий..." required="required"></textarea>
          </div>
          <div class="form-popup_button">
            <input class="webform-submit button-primary form-submit ajax-processed" type="submit" value="Отправить заявку" name="op">
          </div>
          <div class="form-popup_descr">Введите ваше имя и телефон. Специалисты Бухгалтерский успех свяжутся с вами в течение 30 минут.</div>
        </form>
      </div>
    </div>
    <div id="form3" class="form-popup">
      <div class="form-popup_wrap form-popup_wrap__callback">
        <form action="#">
          <div class="form-popup_title">Заказать обратный звонок</div>
          <div class="form-popup_inner">
            <input class="form-text" type="text" maxlength="128" size="60" value="" placeholder="Введите имя">
          </div>
          <div class="form-popup_inner">
            <input class="form-text" type="text" maxlength="128" size="60" value="" placeholder="Введите телефон" required="required">
          </div>
          <div class="form-popup_button">
            <input class="webform-submit button-primary form-submit ajax-processed" type="submit" value="Заказать звонок" name="op">
          </div>
          <div class="form-popup_descr">Введите ваше имя и телефон. Специалисты Бухгалтерский успех свяжутся с вами в течение 30 минут.</div>
        </form>
      </div>
    </div>
  </div>
</body>
</html>